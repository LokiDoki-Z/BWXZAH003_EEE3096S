...
uint32_t adc_val;
uint32_t timeOfLastPress = 0; //stores timestamp of last button press
...
int main(void)
{
  ...
  while (1)
  {
	...
	// ADC to LCD; TODO: Read POT1 value and write to LCD
      ...
      char lcdString[16]; //buffer to store string displayed on LCD

    //Convert ADC value to string
    sprintf(lcdString,  "ADC Val: %lu", adc_val);
    // Display ADC val LCD
     writeLCD(lcdString);

	// Update PWM value; TODO: Get CRR
   
  uint32_t CCR = ADCtoCCR(adc_val); //converts ADC val to CCR (duty cycle) value
...
void EXTI0_1_IRQHandler(void)
{
	// TODO: Add code to switch LED7 delay frequency
	  //check if EXTI0 interrupt flag is set (if button pressed)
    if (__HAL_GPIO_EXTI_GET_IT(Button0_Pin) != RESET)
    { 
        uint32_t currentTime = HAL_GetTick();//get the current time
        uint32_t debounce_time = 100; //debounce period
        
        //checks if enough time has passed since last button press
        if (currentTime - timeOfLastPress >= debounce_time)
        {
            //toggle LED7 frequency between 1 Hz and 2 Hz
            if (delay_t == 1000){ // 1 Hz
                delay_t = 500;  //2 Hz        
            }
            else{ //if 2 Hz
                delay_t = 1000; // 1 Hz               
            }
            //uppdate last button press time as current time
...
void writeLCD(char *char_in){
    ...
    // clear LCD and write string to LCD
    lcd_command(CLEAR);
...
uint32_t pollADC(void){
  ...
  // Starts ADC conversion
    ...
    {
        //Read converted ADC value and store as uint32
...
uint32_t ADCtoCCR(uint32_t adc_val){
  ...
  // 
   uint32_t CCR = (adc_val * (47999 +1)) /4096; // adc_val is between 0 and 4095 (inclusive)
    return CCR;
