...
// TODO: Define any input variables
 uint8_t patterns[] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111};
 uint16_t arrLen = sizeof(patterns);
 uint16_t curr_Index = 0;
 uint16_t currentDelay = 500; //set to 500 so it can set to default 1000 in the button check code
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim16); //starts timer 16 in Interrupt mode

  // TODO: Write all "patterns" to EEPROM using SPI
  uint16_t startAddress=0x0000; //starting address in the EEPROM
    for (uint16_t i=0; i<6; i++) {
        write_to_address(startAddress+i, patterns[i]);
        //add delay
        for(i=1;i<100;i++); //loop creates delay to ensure everything is written to EEPROM
   }
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0)==GPIO_PIN_SET) { //checks button press
	              //switches between 1s and 0.5s delays
	              if(currentDelay ==1000){ //default is 1s
	                 currentDelay =500;
	              }

	              else{
	                  currentDelay =1000; //ensures default is 1s
	              }
	              htim16.Instance->ARR =currentDelay;// change TIM16 delay time
	              // waits for button release to avoid several changes at once
...
void TIM16_IRQHandler(void)
{
	...
	// TODO: Change to next LED pattern; output 0x01 if the read SPI data is incorrect
	        ...
	        uint8_t readVal=read_from_address(curr_Index);

	        if (readVal==patterns[curr_Index]){
	        	GPIOB -> ODR =readVal; //display pattern on leds
	        	if (curr_Index<arrLen-1){
	            curr_Index =(curr_Index +1);
	        	}
	        	else{curr_Index = 0;}
	        }
	        else {
	            // 0b00000001 is output to LEDs if an SPI failure occurs
